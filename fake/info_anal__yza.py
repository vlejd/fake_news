...
def split_and_sanitize(articles):
    res = []
    for a in articles:
        alphanum = re.sub(r'[^a-zA-Z0-9áéíĺóŕúýčďľňšťžäô ]', '', a)
...
split = len(infosk)
indexed, dic, _ = indexify(infosk + hlavne)
infosk = indexed[:split]
hlavne = indexed[split:]
f = open('./data/hlavne_indexed.json', 'w')
f.write(json.dumps(hlavne, indent=4, ensure_ascii=False))
f.close()
f = open('./data/info_indexed.json', 'w')
f.write(json.dumps(infosk, indent=4, ensure_ascii=False))
f.close()
